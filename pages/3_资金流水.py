...
if ff_count == 0:
    st.warning(f"账户「{selected['name']}」暂无资金流水数据，请先上传资金明细单。")
    st.stop()
...
search_keyword = col2.text_input("关键词搜索", placeholder="如：转入、买入、分红...")
...
fund_flows = db.get_fund_flows(account_id, start_date=start_date, end_date=end_date)
...
if fund_flows.empty:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=daily["flow_date"], y=daily["inflow"], name="流入", marker_color="#16a34a"))
    fig.add_trace(go.Bar(x=daily["flow_date"], y=daily["outflow"], name="流出", marker_color="#dc2626"))
    fig.add_trace(go.Scatter(
        x=daily["flow_date"], y=daily["net"],
        name="净额", line=dict(color="#2563eb", width=2), mode="lines+markers",
    ))
    fig.update_layout(
        barmode="relative",
        xaxis_title="日期",
